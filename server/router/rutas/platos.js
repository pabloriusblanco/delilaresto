...
router.route('/') ///Traer platos o crear platos

    .get(async (req, res) => { /// Ver todos los platos. No hace falta estar logeado
    ...
    .post(middlewares.jwtAut, async function (req, res) { ///Crear platos. Solo Admin
...
////PLATOS/:id
router.route('/:id') ///////// GET Todos. PUT y DELETE solo admin.
    .get(async (req, res) => {
...
//esos datos que se hayan pasado en el req.body que si van a haberse actualizado.
    ...
    .delete(middlewares.jwtAut, middlewares.autorizUsuario, async (req, res) => {
        ...
        if (usuarioLogeado.admin == 1) {
            ...
            if (platoEliminado.length == 1) { /// Por alguna razon trae un objeto aunque no existe el ID. El objeto tiene dos objetos dentro. Asi que puedo definir que sea uno solo.
